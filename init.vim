...
" roley add for himself below
call dein#add('easymotion/vim-easymotion')
call dein#add('mileszs/ack.vim')
call dein#add('rking/ag.vim')
call dein#add('chusiang/vim-sdcv.git')
...
" No need for ex mode
nnoremap Q <nop>
" recording inimacros is not my thing
" map q <Nop>
...
let g:unite_prompt='❯ '
" let g:unite_source_rec_async_command =['ag', '--follow', '--nogroup','--hidden', '-g', '', '--ignore', '.git', '--ignore', '*.png', '--ignore', 'lib']
...
nnoremap <silent> <leader>m :Unite -auto-resize -start-insert -direction=botright file_mru<CR>
nnoremap <silent> <leader>d :Unite -auto-resize -start-insert -direction=botright -buffer-name=files -default-action=lcd neomru/directory<CR>
...
" confirm low case and up case
let g:unite_enable_ignore_case = 1
let g:unite_enable_smart_case = 1

" grep search
nnoremap <silent> <leader>k :Unite grep:. -auto-resize -direction=botright -buffer-name=search-buffer<CR>
" nnoremap <silent> <leader>g :<C-u>Unite grep:. -buffer-name=search-buffer<CR>

" search the word under cursor
nnoremap <silent> <leader>cg :Unite grep:. -auto-resize -direction=botright -buffer-name=search-buffer<CR><C-R><C-W>

" grep reload the search result
nnoremap <silent> <leader>r :UniteResume -auto-resize -direction=botright search-buffer<CR>
...
nnoremap <silent> <leader>y :Unite history/yank -auto-resize -direction=botright -default-action=append<CR>

" LIST BUFFER
nnoremap <silent> <leader>b :Unite -auto-resize -direction=botright -buffer-name=buffer buffer<CR>
" LIST FILES
nnoremap <silent> <leader>cb :Unite -auto-resize -direction=botright file <CR>
...
function! neomake#makers#ft#typescript#tsc()
  return {
        \ 'args': [
        \ '-m', 'commonjs', '--target', 'es5', '--emitDecoratorMetadata', 'true', '--experimentalDecorators', 'true' , '--noEmit'
        \ ],
        \ 'errorformat':
        \ '%f %#(%l\,%c): error %m,'
        \ }
...
" Atags and tagbar add here -------------------------------------------------{{{
" if need generate for other language, then change ag --ts --js
let g:atags_build_commands_list = [
      \ "ag --ts --js -g '' | ctags -L - --fields=+l -f tags.tmp",
...
map <silent><Leader>ct :call atags#generate()<cr>
" tagbar below
...
let g:rooter_patterns = ['Rakefile', '.git/', '.gitignore']
...
" xclip simple workaround for x11 systems -----------------------------------{{{
set clipboard+=unnamedplus
...
" endfunction
"
" vnoremap <silent> c y:call ClipboardYank()<cr>
" vnoremap <silent> c d:call ClipboardYank()<cr>
" nnoremap <silent> p c:call ClipboardPaste()<cr>p
" }}}
...
" All tips add here ---------------------------------------------------------{{{
" Git history view use command: Gblame
" git command: ,g
